...
- [~] **Automated tests — started.** Unit tests for the deterministic logic now run on Node's built-in runner (`pnpm test`, no deps): `check-answer`, `answer-format`, `result-band`, `progress`. *Remaining:* `diagnostic`/`practice` units (blocked by extensionless imports without a bundler/loader) and auth/role + RLS smoke tests.
