...
- [~] **Automated tests — started.** Unit tests for the deterministic logic run on Node's built-in runner (`pnpm test`, no deps): `check-answer`, `answer-format`, `result-band`, `progress`, `teacher-resources` (generator validation + selection), and `marketing/plans` (beta-lead validators) — 45 tests. *Remaining:* `diagnostic`/`practice` units (blocked by extensionless imports without a bundler/loader) and auth/role + RLS smoke tests.
