...
**Quiz abuse/storage bounds:** submitted answers are capped at 500 chars in the browser (`maxLength`), the Server Actions (rejected before grading/persistence), and the DB (`attempts_answer_len_ck`). A learner cannot pile up unbounded issued sessions: `startSession` reuses a matching active session of the same type/topic/grade only within a 2-minute grace window (so a refresh or double-click resumes), **supersedes** an older abandoned match (expires it and issues a freshly-selected set — answers only live in the browser, so there is nothing to resume), and otherwise enforces a cap of `MAX_ACTIVE_ISSUED_SESSIONS` (10) simultaneously-issued sessions; superseded matches free their slot. Idempotent finalisation and retries are unchanged.
