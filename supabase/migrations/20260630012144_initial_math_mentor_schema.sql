...
insert into public.topics (grade, name, slug, description, display_order) values
  (9, 'Factorisation', 'factorisation', 'Common factors and simple trinomials.', 1),
  (9, 'Linear equations', 'linear-equations', 'Solve equations one step at a time.', 2),
  (9, 'Exponents', 'exponents', 'Apply the laws of indices.', 3),
  (9, 'Number patterns', 'number-patterns', 'Find rules and predict terms.', 4),
  (10, 'Factorisation', 'factorisation', 'Trinomials, grouping, and difference of squares.', 1),
  (10, 'Algebraic fractions', 'algebraic-fractions', 'Simplify and solve rational expressions.', 2),
  (10, 'Simultaneous equations', 'simultaneous-equations', 'Solve pairs of equations algebraically.', 3),
  (10, 'Functions basics', 'functions', 'Work with notation, inputs, and outputs.', 4),
  (10, 'Exam-style revision', 'exam-revision', 'Mixed algebra questions in exam format.', 5);
...
insert into public.questions (topic_id, grade, question_text, answer_text, hint, solution_steps, difficulty, marks)
select t.id, s.grade, s.question_text, s.answer_text, s.hint, s.steps, s.difficulty, s.marks
from seed s join public.topics t on t.grade = s.grade and t.slug = s.slug;
